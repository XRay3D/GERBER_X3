...
public:
    ...
    enum CommonCodes {
        ...
        StartOfApplicationDefinedGroup = 102, // 	Начало определенной приложением группы u"{имя_приложения"_s. Например, u"{ACAD_REACTORS"_s обозначает начало группы постоянных реакторов AutoCAD (необязательно)
        ...
        IndicatesTheStartOfTheAutocadPersistentReactorsGroup = 102, // 	u"{ACAD_REACTORS"_s обозначает начало группы постоянных реакторов AutoCAD. Эта группа присутствует, только если постоянные реакторы были присоединены к данному объекту (необязательно)
        HandleToOwnerDictionary = 330,                              // 	Идентификатор/дескриптор символьного указателя на словарь владельца (необязательно)
        IndicatesEndOfGroup = 102,                                  // 	Конец группы, u"}"_s (необязательно)
        StartOfAnExtensionDictionaryGrou = 102,                     // 	u"{ACAD_XDICTIONARY"_s обозначает начало группы словаря расширений. Эта группа присутствует, только если к данному объекту были присоединены постоянные реакторы (необязательно)
        ...
        DictionaryEndOfGroup = 102,                                 // 	Конец группы, u"}"_s (необязательно)
