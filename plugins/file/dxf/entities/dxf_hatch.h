...
struct Hatch final : Entity {
    ...
    enum DataEnum {
        ...
        // = Различается , // Данные траекторий контуров. Повторяется столько раз, сколько задано кодом 91. См. u"Данные траекторий контуров"_s
        HatchStyle = 75, // Стиль штриховки:
        // 0 = Hatch “odd parity” area  =  , // 0 = область штриховки по принципу u"кратность двум"_s (обычный стиль)
        ...
        // = Различается , // Данные линий образца. Повторяется столько раз, сколько задано кодом 78. См. u"Данные образца"_s
        ...
        GradientDefinition = 461,                  // Определение градиента; соответствует параметру u"По центру"_s на вкладке u"Градиент"_s диалогового окна u"Штриховка и заливка контура"_s. Каждый градиент имеет два определения: со сдвигом и без. Значение сдвига описывает сглаживание двух определений, которые должны использоваться. Значение 0,0 означает, что следует использовать только версию без сдвига, а значение 1,0 означает, что следует использовать только версию со сдвигом.
