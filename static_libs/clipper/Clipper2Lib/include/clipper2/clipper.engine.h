...
private:
    ...
    std::priority_queue<int_t> scanline_list_;
    std::vector<IntersectNode> intersect_nodes_;
    std::vector<Joiner*> joiner_list_; // pointers in case of memory reallocs
    void Reset();
    void InsertScanline(int_t y);
    bool PopScanline(int_t& y);
    bool PopLocalMinima(int_t y, LocalMinima*& local_minima);
    ...
    void InsertLocalMinimaIntoAEL(int_t bot_y);
    ...
    inline void AdjustCurrXAndCopyToSEL(const int_t top_y);
    void DoIntersections(const int_t top_y);
    void AddNewIntersectNode(Active& e1, Active& e2, const int_t top_y);
    bool BuildIntersectList(const int_t top_y);
    ...
    bool ResetHorzDirection(const Active& horz, const Active* max_pair,
        int_t& horz_left, int_t& horz_right);
    void DoTopOfScanbeam(const int_t top_y);
...
public:
    ...
    PolyPathD* AddChild(const Path64& path) override {
        ...
        result->polygon_ = ScalePath<double, int_t>(path, inv_scale_);
        return result;
    }
...
public:
    ...
    void AddSubject(const PathsD& subjects) {
        AddPaths(ScalePaths<int_t, double>(subjects, scale_), PathType::Subject, false);
    }

    void AddOpenSubject(const PathsD& open_subjects) {
        AddPaths(ScalePaths<int_t, double>(open_subjects, scale_), PathType::Subject, true);
    }

    void AddClip(const PathsD& clips) {
        AddPaths(ScalePaths<int_t, double>(clips, scale_), PathType::Clip, false);
    }
