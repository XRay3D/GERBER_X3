...
inline PathsD InflatePaths(const PathsD& paths, double delta,
    ...
    ClipperOffset clip_offset(miter_limit);
    clip_offset.AddPaths(ScalePaths<int_t, double>(paths, scale), jt, et);
    Paths64 tmp = clip_offset.Execute(delta * scale);
    return ScalePaths<double, int_t>(tmp, 1 / scale);
}

inline Path64 TranslatePath(const Path64& path, int_t dx, int_t dy) {
    Path64 result;
...
inline Paths64 TranslatePaths(const Paths64& paths, int_t dx, int_t dy) {
    Paths64 result;
...
inline PathD RectClip(const RectD& rect, const PathD& path, int precision = 2) {
    ...
    CheckPrecision(precision);
    const double scale = std::pow(10, precision);
    Rect64 r = ScaleRect<int_t, double>(rect, scale);
    class RectClip rc(r);
    Path64 p = ScalePath<int_t, double>(path, scale);
    return ScalePath<double, int_t>(rc.Execute(p), 1 / scale);
}
...
inline PathsD RectClip(const RectD& rect, const PathsD& paths, int precision = 2) {
    ...
    for(const PathD& path: paths) {
        ...
        if(!rect.Intersects(pathRec))
            ...
            p = rc.Execute(p);
            if(!p.empty())
                result.push_back(ScalePath<double, int_t>(p, 1 / scale));
        }
    }
    return result;
}
...
inline PathsD RectClipLines(const RectD& rect, const PathD& path, int precision = 2) {
    ...
    Path64 p = ScalePath<int_t, double>(path, scale);
    return ScalePaths<double, int_t>(rcl.Execute(p), 1 / scale);
}
...
inline PathsD RectClipLines(const RectD& rect, const PathsD& paths, int precision = 2) {
    ...
    for(const PathD& path: paths) {
        ...
        if(!rect.Intersects(pathRec))
            ...
            if(pp.empty())
                continue;
            PathsD ppd = ScalePaths<double, int_t>(pp, 1 / scale);
...
inline bool GetInt(std::string::const_iterator& iter, const std::string::const_iterator& end_iter, int_t& val) {
    val = 0;
    bool is_neg = *iter == '-';
    if(is_neg)
        ++iter;
    ...
    while(iter != end_iter && ((*iter >= '0') && (*iter <= '9')))
        val = val * 10 + (static_cast<int_t>(*iter++) - '0');
    if(is_neg)
        val = -val;
    return (iter != start_iter);
}

inline bool GetFloat(std::string::const_iterator& iter, const std::string::const_iterator& end_iter, double& val) {
    val = 0;
    bool is_neg = *iter == '-';
    if(is_neg)
        ++iter;
    ...
    while(iter != end_iter && (*iter == '.' || ((*iter >= '0') && (*iter <= '9')))) {
        if(*iter == '.') {
            if(dec_pos != 1)
                break;
            dec_pos = 0;
...
inline void SkipUserDefinedChars(std::string::const_iterator& iter,
    ...
    while(iter != end_iter && (*iter <= ' ' || has_one_match(*iter, buff)))
        ++iter;
    return;
}
...
inline PathD MakePathD(const std::string& s) {
    ...
    while(s_iter != s.cend()) {
        double y = 0, x = 0;
        if(!details::GetFloat(s_iter, s.cend(), x))
            break;
        details::SkipSpacesWithOptionalComma(s_iter, s.cend());
        if(!details::GetFloat(s_iter, s.cend(), y))
            break;
...
inline PathD TrimCollinear(const PathD& path, int precision, bool is_open_path = false) {
    ...
    return ScalePath<double, int_t>(p, 1 / scale);
}
