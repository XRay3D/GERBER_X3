...
inline int_t TopX(const Active& ae, const int_t currentY) {
    if((currentY == ae.top.y) || (ae.top.x == ae.bot.x))
        return ae.top.x;
    else if(currentY == ae.bot.y)
        return ae.bot.x;
    else
        return ae.bot.x + static_cast<int_t>(std::nearbyint(ae.dx * (currentY - ae.bot.y)));
...
void ClipperBase::CleanUp() {
    ...
    scanline_list_ = std::priority_queue<int_t>();
...
inline void ClipperBase::InsertScanline(int_t y) {
    scanline_list_.push(y);
}

bool ClipperBase::PopScanline(int_t& y) {
...
bool ClipperBase::PopLocalMinima(int_t y, LocalMinima*& local_minima) {
...
void ClipperBase::InsertLocalMinimaIntoAEL(int_t bot_y) {
...
inline void ClipperBase::AdjustCurrXAndCopyToSEL(const int_t top_y) {
    Active* e = actives_;
    sel_ = e;
    while(e) {
...
void ClipperBase::DoIntersections(const int_t top_y) {
...
void ClipperBase::AddNewIntersectNode(Active& e1, Active& e2, int_t top_y) {
    Point64 ip;
...
bool ClipperBase::BuildIntersectList(const int_t top_y) {
...
bool ClipperBase::ResetHorzDirection(const Active& horz,
    const Active* max_pair, int_t& horz_left, int_t& horz_right) {
                ...
                if(e->curr_x == horz.top.x && !IsHorizontal(*e)) {
                    ...
                    if(is_left_to_right) {
                        ...
                        // otherwise we'll only break when horz's outslope is greater than e's
                            ...
                            break;
                    } else if(IsOpen(*e) && !IsSamePolyType(*e, horz) && !IsHotEdge(*e)) {
                        if(TopX(*e, pt.y) < pt.x)
                            break;
                    } else if(TopX(*e, pt.y) <= pt.x)
                        break;
...
void ClipperBase::DoTopOfScanbeam(const int_t y) {
    ...
    while(joiner) {
        if(joiner->op1 == op)
            if(joiner->next1 && joiner->next1->idx < 0)
                ...
                joiner = joiner->next1;
        else if(joiner->next2 && joiner->next2->idx < 0)
            return joiner;
        else
            joiner = joiner->next1;
    }
...
inline bool HorzEdgesOverlap(int_t x1a, int_t x1b, int_t x2a, int_t x2b) {
    const int_t minOverlap = 2;
...
inline bool ValueBetween(int_t val, int_t end1, int_t end2) {
    // NB accommodates axis aligned between where end1 == end2
    return ((val != end1) == (val != end2)) && ((val > end1) == (val < end2));
}

inline bool ValueEqualOrBetween(int_t val, int_t end1, int_t end2) {
    return (val == end1) || (val == end2) || ((val > end1) == (val < end2));
}
