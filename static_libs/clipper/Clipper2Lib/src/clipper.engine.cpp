...
static const RectI invalid_rect = RectI(
...
inline IntType TopX(const Active& ae, const IntType currentY) {
    if((currentY == ae.top.y) || (ae.top.x == ae.bot.x))
        return ae.top.x;
    else if(currentY == ae.bot.y)
        return ae.bot.x;
    else
        return ae.bot.x + static_cast<IntType>(std::nearbyint(ae.dx * (currentY - ae.bot.y)));
...
inline OutPt* InsertOp(const PointI& pt, OutPt* insertAfter) {
...
inline double AreaTriangle(const PointI& pt1,
                           const PointI& pt2, const PointI& pt3) {
    return (static_cast<double>(pt3.y + pt1.y) * static_cast<double>(pt3.x - pt1.x) + static_cast<double>(pt1.y + pt2.y) * static_cast<double>(pt1.x - pt2.x) + static_cast<double>(pt2.y + pt3.y) * static_cast<double>(pt2.x - pt3.x));
}
...
inline bool TestJoinWithPrev1(const Active& e) {
    ...
    return IsHotEdge(e) && !IsOpen(e) && e.prev_in_ael && e.prev_in_ael->curr_x == e.curr_x && IsHotEdge(*e.prev_in_ael) && !IsOpen(*e.prev_in_ael) && (CrossProduct(e.prev_in_ael->top, e.bot, e.top) == 0);
}

inline bool TestJoinWithPrev2(const Active& e, const PointI& curr_pt) {
    return IsHotEdge(e) && !IsOpen(e) && e.prev_in_ael && !IsOpen(*e.prev_in_ael) && IsHotEdge(*e.prev_in_ael) && (e.prev_in_ael->top.y < e.bot.y) && (std::llabs(TopX(*e.prev_in_ael, curr_pt.y) - curr_pt.x) < 2) && (CrossProduct(e.prev_in_ael->top, curr_pt, e.top) == 0);
}
...
inline bool TestJoinWithNext1(const Active& e) {
    ...
    return IsHotEdge(e) && !IsOpen(e) && e.next_in_ael && (e.next_in_ael->curr_x == e.curr_x) && IsHotEdge(*e.next_in_ael) && !IsOpen(*e.next_in_ael) && (CrossProduct(e.next_in_ael->top, e.bot, e.top) == 0);
}

inline bool TestJoinWithNext2(const Active& e, const PointI& curr_pt) {
    return IsHotEdge(e) && !IsOpen(e) && e.next_in_ael && !IsOpen(*e.next_in_ael) && IsHotEdge(*e.next_in_ael) && (e.next_in_ael->top.y < e.bot.y) && (std::llabs(TopX(*e.next_in_ael, curr_pt.y) - curr_pt.x) < 2) && (CrossProduct(e.next_in_ael->top, curr_pt, e.top) == 0);
}
...
void ClipperBase::CleanUp() {
    ...
    scanline_list_ = std::priority_queue<IntType>();
...
void ClipperBase::AddPaths(const PathsI& paths, PathType polytype, bool is_open) {
    for(auto&& path : paths)
        ProgressCancel::setMax(ProgressCancel::max() + path.size());
    if(is_open)
        has_open_paths_ = true;
    minima_list_sorted_ = false;

    PathI::size_type total_vertex_count = 0;
...
inline void ClipperBase::InsertScanline(IntType y) {
    scanline_list_.push(y);
}

bool ClipperBase::PopScanline(IntType& y) {
...
bool ClipperBase::PopLocalMinima(IntType y, LocalMinima*& local_minima) {
...
bool IsValidAelOrder(const Active& resident, const Active& newcomer) {
    ...
    if(!IsMaxima(resident) && (resident.top.y > newcomer.top.y)) {
        return CrossProduct(newcomer.bot,
                            resident.top, NextVertex(resident)->pt)
            <= 0;
    } else if(!IsMaxima(newcomer) && (newcomer.top.y > resident.top.y)) {
        return CrossProduct(newcomer.bot,
                            newcomer.top, NextVertex(newcomer)->pt)
            >= 0;
    }

    IntType y = newcomer.bot.y;
...
void ClipperBase::InsertLocalMinimaIntoAEL(IntType bot_y) {
...
bool ClipperBase::ValidateClosedPathEx(OutPt*& outpt) {
    if(IsValidClosedPath(outpt))
        return true;
    if(outpt)
        SafeDisposeOutPts(outpt);
    return false;
}

void ClipperBase::CleanCollinear(OutRec* outrec) {
    outrec = GetRealOutRec(outrec);
    if(!outrec || outrec->is_open || outrec->front_edge || !ValidateClosedPathEx(outrec->pts))
        return;
    ...
    for(;;) {
        ...
        if((CrossProduct(op2->prev->pt, op2->pt, op2->next->pt) == 0) && (op2->pt == op2->prev->pt || op2->pt == op2->next->pt || !PreserveCollinear || DotProduct(op2->prev->pt, op2->pt, op2->next->pt) < 0)) {
...
OutPt* ClipperBase::IntersectEdges(Active& e1, Active& e2, const PointI& pt) {
    ...
    if(IsHotEdge(e1) && IsHotEdge(e2)) {
        if((old_e1_windcnt != 0 && old_e1_windcnt != 1) || (old_e2_windcnt != 0 && old_e2_windcnt != 1) || (e1.local_min->polytype != e2.local_min->polytype && cliptype_ != ClipType::Xor)) {
            ...
            if(zCallback_)
                SetZ(e1, e2, op2->pt);
#endif
            if(resultOp && resultOp->pt == op2->pt && !IsHorizontal(e1) && !IsHorizontal(e2) && (CrossProduct(e1.bot, resultOp->pt, e2.bot) == 0))
        ...
        if(!IsSamePolyType(e1, e2)) {
            ...
            case ClipType::Union:
                ...
                break;
            case ClipType::Difference:
                if(((GetPolyType(e1) == PathType::Clip) && (e1Wc2 > 0) && (e2Wc2 > 0)) || ((GetPolyType(e1) == PathType::Subject) && (e1Wc2 <= 0) && (e2Wc2 <= 0))) {
                    resultOp = AddLocalMinPoly(e1, e2, pt, false);
                }
                break;
            case ClipType::Xor:
...
inline void ClipperBase::AdjustCurrXAndCopyToSEL(const IntType top_y) {
    Active* e = actives_;
    sel_ = e;
    while(e) {
...
void ClipperBase::DoIntersections(const IntType top_y) {
...
void ClipperBase::AddNewIntersectNode(Active& e1, Active& e2, IntType top_y) {
    PointI ip;
...
bool ClipperBase::BuildIntersectList(const IntType top_y) {
...
bool ClipperBase::ResetHorzDirection(const Active& horz,
                                     const Active* max_pair, IntType& horz_left, IntType& horz_right) {
...
inline bool HorzIsSpike(const Active& horzEdge) {
    PointI nextPt = NextVertex(horzEdge)->pt;
 ...
 *******************************************************************************/
{
...
#else
        AddOutPt(horz, PointI(horz.curr_x, y));
#endif
    ...
    while(true) // loop through consec. horizontal edges
    {
        ...
        while(e) {
            ProgressCancel::ifCancelThenThrow();
            if(e == max_pair) {
                if(IsHotEdge(horz)) {
            ...
            if(vertex_max != horz.vertex_top || IsOpenEnd(horz)) {
                ...
                if((is_left_to_right && e->curr_x > horz_right) || (!is_left_to_right && e->curr_x < horz_left))
                    break;
    ...
    if((horzIsOpen && !IsOpenEnd(horz)) || (!horzIsOpen && vertex_max != horz.vertex_top)) {
...
void ClipperBase::DoTopOfScanbeam(const IntType y) {
...
Joiner* ClipperBase::GetHorzTrialParent(const OutPt* op) {
    ...
    while(joiner) {
        if(joiner->op1 == op) {
            if(joiner->next1 && joiner->next1->idx < 0)
                return joiner;
            else
                joiner = joiner->next1;
        } else {
            if(joiner->next2 && joiner->next2->idx < 0)
                return joiner;
            else
                joiner = joiner->next1;
        }
...
inline bool HorzEdgesOverlap(IntType x1a, IntType x1b, IntType x2a, IntType x2b) {
    const IntType minOverlap = 2;
...
inline bool ValueBetween(IntType val, IntType end1, IntType end2) {
    // NB accommodates axis aligned between where end1 == end2
    return ((val != end1) == (val != end2)) && ((val > end1) == (val < end2));
}

inline bool ValueEqualOrBetween(IntType val, IntType end1, IntType end2) {
    return (val == end1) || (val == end2) || ((val > end1) == (val < end2));
}

inline bool PointBetween(PointI pt, PointI corner1, PointI corner2) {
    // NB points may not be collinear
    return ValueBetween(pt.x, corner1.x, corner2.x) && ValueBetween(pt.y, corner1.y, corner2.y);
}

inline bool PointEqualOrBetween(PointI pt, PointI corner1, PointI corner2) {
    // NB points may not be collinear
...
void ClipperBase::ConvertHorzTrialsToJoins() {
    while(horz_joiners_) {
        ...
        while(joiner) {
            ...
            if(GetHorzExtendedHorzSeg(op2a, op2b) && HorzEdgesOverlap(op1a->pt.x, op1b->pt.x, op2a->pt.x, op2b->pt.x)) {
...
bool CheckDisposeAdjacent(OutPt*& op, const OutPt* guard, OutRec& outRec) {
    bool result = false;
    while(op->prev != op) {
        if(op->pt == op->prev->pt && op != guard && op->prev->joiner && !op->joiner) {
            if(op == outRec.pts)
                outRec.pts = op->prev;
            op = DisposeOutPt(op);
            op = op->prev;
    ...
    while(op->next != op) {
        if(op->pt == op->next->pt && op != guard && op->next->joiner && !op->joiner) {
            if(op == outRec.pts)
                outRec.pts = op->prev;
            op = DisposeOutPt(op);
            op = op->prev;
...
bool CollinearSegsOverlap(const PointI& seg1a, const PointI& seg1b,
                          const PointI& seg2a, const PointI& seg2b) {
...
OutRec* ClipperBase::ProcessJoin(Joiner* joiner) {
    ...
    if(or2->pts == nullptr)
        ...
        SafeDisposeOutPts(op1);
        return or2;
    } else if(or1 == or2 && ((op1 == op2) || (op1->next == op2) || (op1->prev == op2)))
        return or1;
    ...
    for(;;) {
        ...
        if(op1->prev->pt == op2->next->pt || ((CrossProduct(op1->prev->pt, op1->pt, op2->next->pt) == 0) && CollinearSegsOverlap(op1->prev->pt, op1->pt, op2->pt, op2->next->pt))) {
            if(or1 == or2) {
                // SPLIT REQUIRED
                ...
                if(or1->idx < or2->idx) {
                    or1->pts = op1;
                    or2->pts = nullptr;
                    if(or1->owner && (!or2->owner || or2->owner->idx < or1->owner->idx))
                        or1->owner = or2->owner;
                    or2->owner = or1;
                } else {
                    result = or2;
                    or2->pts = op1;
                    or1->pts = nullptr;
                    if(or2->owner && (!or1->owner || or1->owner->idx < or2->owner->idx))
                        or2->owner = or1->owner;
                    or1->owner = or2;
                }
            }
            break;
        } else if(op1->next->pt == op2->prev->pt || ((CrossProduct(op1->next->pt, op2->pt, op2->prev->pt) == 0) && CollinearSegsOverlap(op1->next->pt, op1->pt, op2->pt, op2->prev->pt))) {
            if(or1 == or2) {
                // SPLIT REQUIRED
            ...
            break;
        } else if(PointBetween(op1->next->pt, op2->pt, op2->prev->pt) && DistanceFromLineSqrd(op1->next->pt, op2->pt, op2->prev->pt) < 2.01) {
            InsertOp(op1->next->pt, op2->prev);
            continue;
        } else if(PointBetween(op2->next->pt, op1->pt, op1->prev->pt) && DistanceFromLineSqrd(op2->next->pt, op1->pt, op1->prev->pt) < 2.01) {
            InsertOp(op2->next->pt, op1->prev);
            continue;
        } else if(PointBetween(op1->prev->pt, op2->pt, op2->next->pt) && DistanceFromLineSqrd(op1->prev->pt, op2->pt, op2->next->pt) < 2.01) {
            InsertOp(op1->prev->pt, op2);
            continue;
        } else if(PointBetween(op2->prev->pt, op1->pt, op1->next->pt) && DistanceFromLineSqrd(op2->prev->pt, op1->pt, op1->next->pt) < 2.01) {
            InsertOp(op2->prev->pt, op1);
            continue;
        }

        // something odd needs tidying up
        if(CheckDisposeAdjacent(op1, op2, *or1))
            continue;
        else if(CheckDisposeAdjacent(op2, op1, *or1))
            continue;
        else if(op1->prev->pt != op2->next->pt && (DistanceSqr(op1->prev->pt, op2->next->pt) < 2.01)) {
            op1->prev->pt = op2->next->pt;
            continue;
        } else if(op1->next->pt != op2->prev->pt && (DistanceSqr(op1->next->pt, op2->prev->pt) < 2.01)) {
...
inline RectI GetBounds(const PathI& path) {
    if(path.empty())
        return RectI();
    RectI result = invalid_rect;
    for(const PointI& pt : path) {
        if(pt.x < result.left)
            result.left = pt.x;
...
bool ClipperBase::DeepCheckOwner(OutRec* outrec, OutRec* owner) {
    ...
    if(owner->splits) {
        for(OutRec* split : *owner->splits) {
            ...
            if(split->splits && DeepCheckOwner(outrec, split))
                return true;
...
void Clipper64::BuildPaths64(PathsI& solutionClosed, PathsI* solutionOpen) {
