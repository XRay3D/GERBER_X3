...
void Constructor::addShapePoint(const QPointF& point)
{
    if (type) {
        if (!item)
            item = interface->createShape(point);
        else if (!interface->addShapePoint(point))
            finalizeShape();
    }
    //    point = value;
...
void Constructor::updateShape(const QPointF& point)
{
    interface->updateShape(point);
    //    point = value;
    //    if (item == nullptr)
    //        return;
    //    switch (static_cast<GiType>(type)) {
