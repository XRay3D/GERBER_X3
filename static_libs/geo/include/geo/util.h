...
// Прореживание контура с гарантированным отклонением не больше tolerance --
// для потребителей, которым нужна ФОРМА с допуском, а не сама граница:
// офсет на дистанцию много больше tolerance (черновые витки кармана) от
// прореженного контура неотличим в пределах допуска, а точному движку
// достаётся в разы меньше кривых.
//
// Два прохода, каждый со своей половиной допуска:
//   * дуга со стрелой (сагиттой) не больше tolerance/2 спрямляется в хорду --
//     заодно её пересечения из алгебраических (Sqrt_extension) становятся
//     рациональными, а они в точном домене ощутимо дешевле;
//   * Дуглас-Пекер по пробегам подряд идущих ПРЯМЫХ сегментов; вершины,
//     смежные с уцелевшими дугами, неприкосновенны, сами дуги не трогаются.
//
// Полная окружность (две вершины, |bulge| == 1) возвращается как есть: её
// офсет и так один диск (см. Offset::fullCircleOf). Если прореженный
// ЗАМКНУТЫЙ контур выходит вырожденным или самокасающимся (тонкая шея уже
// допуска -- isExactContour его бракует, и регион молча потерял бы целый
// контур), возвращается ИСХОДНЫЙ контур -- прореживание здесь оптимизация,
// терять геометрию ей нельзя.
