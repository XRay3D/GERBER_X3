...
public:
    ...
    //
    // coarse > 0 -- допуск НЕДОБОРА: полоса может оказаться тоньше честной,
    // но не больше чем на coarse, и никогда не толще. Взамен кусков границы
    // в объединение уходит тем меньше, чем больше radius (мелочь границы
    // накрывается редкими дисками, см. Cgal::boundaryCapsules), а цена
    // точного объединения почти линейна по их числу. Для потребителя это
    // офсет, смещённый К ГРАНИЦЕ не дальше coarse: черновые витки кармана
    // такой допуск покрывают запасом, чистовым он не предлагается.
    Polygons boundaryBand(double radius, double coarse = 0.0) const;
