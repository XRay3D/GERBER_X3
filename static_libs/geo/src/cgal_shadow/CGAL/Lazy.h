...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.2/Filtered_kernel/include/CGAL/Lazy.h $
// $Id: include/CGAL/Lazy.h cac3e9d75e2 $
// SPDX-License-Identifier: LGPL-3.0-or-later OR LicenseRef-Commercial
//
//
// Author(s)     : Andreas Fabri, Sylvain Pion
//
// ИЗМЕНЕНО (GERBER_X3, теневая копия поверх CGAL 6.2, см. cgal_shadow в
// static_libs/geo/CMakeLists.txt): std::call_once в Lazy_rep::exact()
// заменён на атомарный gx3_lazy_once. На MinGW libstdc++ собран без futex,
// и его generic-реализация call_once ходит в ГЛОБАЛЬНЫЙ мьютекс с
// глобальной же condvar (notify_all = SetEvent) на КАЖДУЮ первую
// эксактификацию узла -- миллионы сисколов на карман и полная
// сериализация всех потоков точной арифметики об один замок (ETW-трасса:
// ~треть сэмплов однопоточного прогона в стеках SetEvent, при 8 потоках
// шторм переключений контекста). Атомарный once с парковкой на
// atomic::wait только при реальной гонке снимает и то и другое.
// Остальное тело файла не тронуто; при обновлении CGAL пересравнить
// (страж версии ниже).
...
#if CGAL_VERSION_NR / 100000 != 10602
#  error "Теневая Lazy.h написана поверх CGAL 6.2 -- пересравните с новой версией и обновите копию."
#endif

namespace CGAL {

// Замена std::call_once для Lazy_rep: 0 -- не считано, 1 -- считается,
// 2 -- готово, 3 -- считается и есть ждущие. Быстрый путь -- один
// acquire-load; завершение будит только при реальных ждущих (значение 3),
// так что в бесспорном случае нет ни одного вызова ядра. Исключение из
// функции возвращает флаг в 0 -- семантика std::call_once.
template <class F>
inline void gx3_lazy_once(std::atomic<unsigned char>& flag, F&& f)
{
  ...
  while (s != 2) {
    if (s == 0 && flag.compare_exchange_weak(s, 1, std::memory_order_acquire)) {
      try {
        ...
        if (flag.exchange(0, std::memory_order_release) == 3) flag.notify_all();
        throw;
      }
      if (flag.exchange(2, std::memory_order_release) == 3) flag.notify_all();
      return;
    }
...
public:
  ...
  mutable std::atomic<unsigned char> once{0}; // GX3: см. gx3_lazy_once выше
  ...
  const ET & exact() const
  {
    ...
    //if (is_lazy())
    gx3_lazy_once(once, [this](){this->update_exact();});
