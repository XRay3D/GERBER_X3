...
// на месте: запуск потока стоит десятки микросекунд, и на горстке коротких
// тел он их не отобьёт. Умолчание подобрано под лёгкие тела (перевод одного
// контура); для тяжёлых (целый офсет региона на элемент, см.
// Geo::InflateSeries) имеет смысл 1.
//
// Вложенный вызов -- из тела, уже исполняющегося внутрь parallelFor или
// joinAll, -- идёт на месте без потоков: ядра уже заняты внешним уровнем,
// второй лишь оверподписал бы их.
void parallelFor(std::size_t count, const std::function<void(std::size_t)>& body,
    std::size_t minBatch = 8);
