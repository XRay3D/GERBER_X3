...
bool Parser::parseNumber(QString Str, cInt& val, int integer, int decimal) {
    ...
    if(!Str.isEmpty()) {
        ...
        while(Str.length() < integer + decimal) {
            ...
            case OmitLeadingZeros: Str = QString(integer + decimal - Str.length(), '0') + Str; // Str = u"0"_s + Str ;break;
#ifdef DEPRECATED
            case OmitTrailingZeros: Str += QString(integer + decimal - Str.length(), '0'); // Str += u"0"_s ;break;
#endif
            }
        }
...
#endif
            break;
        default: qWarning() << "Erroror or deprecated G-code " << match.capturedTexts(); break;
        }
        return true;
    }
