...
bool FixOrientation(Active& e)
{
    ...
    while (e2->prev_in_ael) {
        e2 = e2->prev_in_ael;
        if (e2->outrec && !IsOpen(*e2))
            result = !result;
    }
    if (result != IsStartSide(e)) {
        if (result)
            e.outrec->flag = orOuter;
        else
            e.outrec->flag = orInner;
        SwapSides(*e.outrec);
...
{
    return (e1.top.y > e2.top.y) ? TopX(e2, e1.top.y) - e1.top.x : e2.top.x - TopX(e1, e2.top.y);
}
//----------------------------------------------------------------------------
...
{
    if (prefer_left) {
        return e2.curr.x == e1.curr.x ? GetTopDeltaX(e1, e2) < 0 : e2.curr.x < e1.curr.x;
    } else
        return e2.curr.x == e1.curr.x ? GetTopDeltaX(e1, e2) <= 0 : e2.curr.x <= e1.curr.x;
}
//------------------------------------------------------------------------------
...
Point64 GetIntersectPoint(const Active& e1, const Active& e2)
{
    ...
    if (e1.dx == 0) {
        ...
        return (std::fabs(e1.dx) < std::fabs(e2.dx)) ? Point64(Round(e1.dx * q + b1), Round(q)) : Point64(Round(e2.dx * q + b2), Round(q));
    }
}
//------------------------------------------------------------------------------
...
{
    if (curr_loc_min_ == minima_list_.end() || (*curr_loc_min_)->vertex->pt.y != y)
        return false;
...
void Clipper::AddPathToVertexList(const Path& path, PathType polytype, bool is_open)
{
    ...
    while (path_len > 1 && (path[path_len - 1] == path[0]))
        --path_len;
    if (path_len < 2)
        return;
    ...
    while ((i < path_len) && (path[i].y == path[0].y))
        ++i;
...
void Clipper::InsertEdgeIntoAEL(Active& e1, Active* e2)
{
    ...
    if (!e2) {
        ...
        e2 = actives_;
        while (e2->next_in_ael && E2InsertsBeforeE1(e1, *e2->next_in_ael, false))
            e2 = e2->next_in_ael;
    } else {
        while (e2->next_in_ael && E2InsertsBeforeE1(e1, *e2->next_in_ael, true))
            e2 = e2->next_in_ael;
    }
...
OutRec* Clipper::GetOwner(const Active* e)
{
    if (IsHorizontal(*e) && e->top.x < e->bot.x) {
        ...
        if (!e)
            return NULL;
        return ((e->outrec->flag == orOuter) == (e->outrec->start_e == e)) ? e->outrec->owner : e->outrec;
    } else {
        e = e->prev_in_ael;
        while (e && (!IsHotEdge(*e) || IsOpen(*e)))
            e = e->prev_in_ael;
        if (!e)
            return NULL;
        return ((e->outrec->flag == orOuter) == (e->outrec->end_e == e)) ? e->outrec->owner : e->outrec;
    }
}
//------------------------------------------------------------------------------
...
void Clipper::IntersectEdges(Active& e1, Active& e2, const Point64 pt)
{
    ...
    if (has_open_paths_ && (IsOpen(e1) || IsOpen(e2))) {
        ...
        case ctIntersection:
        case ctDifference:
            if (IsSamePolyType(*edge_o, *edge_c) || (Abs(edge_c->wind_cnt) != 1))
                return;
            break;
        case ctUnion:
            if (IsHotEdge(*edge_o) != ((Abs(edge_c->wind_cnt) != 1) || (IsHotEdge(*edge_o) != (edge_c->wind_cnt != 0))))
                return; //just works!
            break;
        case ctXor:
    ...
    if (IsHotEdge(e1) && IsHotEdge(e2)) {
        if ((old_e1_windcnt != 0 && old_e1_windcnt != 1) || (old_e2_windcnt != 0 && old_e2_windcnt != 1) || (e1.local_min->polytype != e2.local_min->polytype && cliptype_ != ctXor)) {
    ...
    } else if ((old_e1_windcnt == 0 || old_e1_windcnt == 1) && (old_e2_windcnt == 0 || old_e2_windcnt == 1)) {
        ...
        if (!IsSamePolyType(e1, e2)) {
            ...
            case ctUnion:
                ...
                break;
            case ctDifference:
                if (((GetPolyType(e1) == ptClip) && (e1Wc2 > 0) && (e2Wc2 > 0)) || ((GetPolyType(e1) == ptSubject) && (e1Wc2 <= 0) && (e2Wc2 <= 0)))
                    AddLocalMinPoly(e1, e2, pt);
                break;
            case ctXor:
                AddLocalMinPoly(e1, e2, pt);
...
void Clipper::FixupIntersectionOrder()
{
    ...
    for (size_t i = 0; i < cnt; ++i) {
        if (!EdgesAdjacentInSel(*intersect_list_[i])) {
            size_t j = i + 1;
            while (!EdgesAdjacentInSel(*intersect_list_[j]))
                j++;
...
void Clipper::SwapPositionsInAEL(Active& e1, Active& e2)
{
    ...
    if (e1.next_in_ael == e1.prev_in_ael || e2.next_in_ael == e2.prev_in_ael)
        return;
...
{
    if (!e1.next_in_sel && !e1.prev_in_sel)
        return;
    if (!e2.next_in_sel && !e2.prev_in_sel)
        return;
    ...
    *******************************************************************************/
{
    ...
    if (IsMaxima(horz) && (!IsOpen(horz) || ((horz.vertex_top->flags & (vfOpenStart | vfOpenEnd)) == 0)))
    ...
    while (true) { //loops through consec. horizontal edges (if open)
        ...
        while (e) {
            ...
            if ((is_left_to_right && (e->curr.x > horz_right)) || (!is_left_to_right && (e->curr.x < horz_left)))
                break;
            //or if we've got to the } of an intermediate horizontal edge ...
            if (e->curr.x == horz.top.x && !isMax && !IsHorizontal(*e)) {
                pt = NextVertex(horz).pt;
                if (is_left_to_right && (TopX(*e, pt.y) >= pt.x) || (!is_left_to_right && (TopX(*e, pt.y) <= pt.x)))
                    break;
            };
...
Rect64 Clipper::GetBounds()
{
    ...
    while (lm_iter != minima_list_.end()) {
        ...
        do {
            if (v2.pt.x < result.left)
                result.left = v2.pt.x;
            if (v2.pt.x > result.right)
                result.right = v2.pt.x;
            if (v2.pt.y < result.top)
                result.top = v2.pt.y;
            if (v2.pt.y > result.bottom)
                result.bottom = v2.pt.y;
        } while (&v2 != &v);
        ++lm_iter;
    }
    return result;
